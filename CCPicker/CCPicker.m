...
-(void)changeActualPageTo:(NSInteger)page{
    [_scrollPicker setActualPage:page];
}
...
-(void)loadItems{
    ...
    if([_pickerDelegate respondsToSelector:@selector(numberOfItemsForPicker:)]){
        _numberOfRows = [_pickerDelegate numberOfItemsForPicker:self];
    }else{
        _numberOfRows = 0;
    }
    ...
    for (int i = 0; i < _numberOfRows; i++) {
        ...
        if (!label) {
            @throw [NSException exceptionWithName:NSObjectNotAvailableException reason:@"Nil label!" userInfo:nil];
        }
